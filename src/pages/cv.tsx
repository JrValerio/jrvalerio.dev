    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-gray-900 flex items-center justify-center print:bg-white print:text-black print:p-0">
      <article className="w-full max-w-3xl mx-auto bg-white/95 dark:bg-[#202531] rounded-2xl shadow-2xl border border-gray-300 px-2 sm:px-8 py-8 sm:py-12 my-8 print:rounded-none print:shadow-none print:border-0 print:bg-white transition-colors">
        <div className="flex gap-2 mb-4 justify-end print:hidden">
          <FaGlobe className="text-gray-500 mt-[2px]" />
          <button className={`font-bold text-xs px-2 py-1 rounded ${i18n.language==='pt'?'bg-teal-500 text-white':'text-gray-700 hover:bg-gray-200 transition-colors'}`} onClick={()=>i18n.changeLanguage('pt')}>PT</button>
          <button className={`font-bold text-xs px-2 py-1 rounded ${i18n.language==='en'?'bg-teal-500 text-white':'text-gray-700 hover:bg-gray-200 transition-colors'}`} onClick={()=>i18n.changeLanguage('en')}>EN</button>
          <button className={`font-bold text-xs px-2 py-1 rounded ${i18n.language==='es'?'bg-teal-500 text-white':'text-gray-700 hover:bg-gray-200 transition-colors'}`} onClick={()=>i18n.changeLanguage('es')}>ES</button>
        </div>
        ...
        <Section title={t('education')}>
          {Array.isArray(educations) && educations.map((edu, i) => (
            <div className="mb-2" key={i}>
              ...
              </strong> <span className="text-gray-500 italic">{edu.date}</span><br />
        ...
        <Section title={t('experience')}>
          {Array.isArray(experiences) && experiences.map((exp, i) => (
            <div className="mb-2" key={i}>
              <strong className="underline text-teal-600 hover:text-blue-500 transition-colors">{exp.title}</strong> <span className="text-gray-500 italic">{exp.date}</span>
              <ul className="list-disc ml-6 text-gray-800 dark:text-gray-200 print:text-black">
                {exp.bullets && exp.bullets.map((b: string, idx: number) => (
                  <li key={idx}>{b}</li>
                ))}
              </ul>
        ...
        <Section title={t('certs')}>
          <ul className="list-disc ml-6 text-gray-800 dark:text-gray-200">
            {Array.isArray(certifications) && certifications.map((c, i) => <li key={i}>{c}</li>)}
          </ul>
        </Section>
        <Section title={t('idioms')}>
          <ul className="list-disc ml-6 text-gray-800 dark:text-gray-200">
            {Array.isArray(idioms) && idioms.map((l, i) => <li key={i}>{l}</li>)}
          </ul>
        </Section>
