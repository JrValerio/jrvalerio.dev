...
2. **LCP 4–9 s** (on throttled CPU) — Phase 3 investigation revealed two layers: (a) `.jr-reveal { opacity: 0 }` excluded `p.jr-hero-summary` from LCP candidacy entirely, making Chrome measure the smaller `102×20 px` brand link instead; (b) once `opacity: 0` was removed, the real LCP candidate (`p.jr-hero-summary`) has a 4–9 s render delay caused by **font loading + WebGL TBT** (94% of the delay per Lighthouse phase breakdown). Motion-based animations were tried but made LCP worse (8–10 s) because motion applies `initial` state inline styles during SSR, keeping the LCP candidate unsettled until animation completes. Current state: `opacity: 0` removed, hero uses CSS transform-only animation (`.jr-slide-in`), LCP candidate restored. See ADR-004 for full investigation log.

The threshold of 0.55 catches genuine regressions without flagging the WebGL + font-loading baseline. **Raise to 0.70 after font optimization** (`font-display: swap` + `<link rel="preload">` for the hero font) reduces hero render delay from 4–9 s → ~1.5 s.
