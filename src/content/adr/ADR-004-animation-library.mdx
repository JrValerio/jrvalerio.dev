...
- Hero LCP candidate (`p.jr-hero-summary`) is now eligible (no `opacity: 0`). Render delay of 4–9 s is caused by font loading + WebGL TBT — not by animation.
- **Real next LCP lever:** `font-display: swap` + `<link rel="preload">` for the hero font. Estimated improvement: 4 s → ~1.5 s. Deferred to a separate commit.
- LHCI performance threshold raise from 0.55 → 0.70 is deferred until font loading is optimised (font loading is 94% of the hero render delay per Lighthouse LCP phase breakdown).
