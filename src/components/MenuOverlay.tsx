...
import ThemeToggle from "./ThemeToggle"; // Importe aqui também!
...
export default function MenuOverlay({ open, onClose }: MenuOverlayProps) {
  ...
  return (
    <div className={`
      fixed inset-0 z-[9999] bg-black/90 dark:bg-white/95 dark:text-gray-900
    ...
    `}>
      ...
      <div className="flex flex-col items-center gap-4 mt-10">
        <div className="flex items-center gap-3">
          <FaGlobe className="text-gray-400 dark:text-gray-600" aria-label="Abrir seletor de idiomas" />
          {["pt", "en", "es"].map((lng) => (
            <button
              key={lng}
